...
// Broadcast a platform-wide push to every app user (FCM topic "all_users").
export async function broadcastAll(title, body, html) {
  ...
  const r = await fetch("/api/broadcast", {
    ...
    body: JSON.stringify({ token, title, body, html }),
  });
...
export function watchOtpDebug(cb) {
  // No orderBy — a doc with a still-pending serverTimestamp would be excluded by
  // orderBy("at"). Sort client-side so every code shows immediately.
  return onSnapshot(
    collection(db, "otpDebug"),
    (snap) => {
...
// Edit community fields from the admin 360° panel.
export const updateCommunityFields = (cid, data) =>
  updateDoc(doc(db, "communities", cid), { ...data, updatedAt: serverTimestamp() });

/* ─────────── Admin: hosts (dashboard accounts) ─────────── */
export function watchHosts(cb) {
  return onSnapshot(
    collection(db, "hosts"),
    (snap) => cb(snap.docs.map((d) => ({ id: d.id, ...d.data() }))),
    () => cb([]),
...
export const setHostSuspended = (uid, suspended) =>
  updateDoc(doc(db, "hosts", uid), { suspended, updatedAt: serverTimestamp() });
export const deleteHost = (uid) => deleteDoc(doc(db, "hosts", uid));
